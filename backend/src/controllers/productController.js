...
const productController = {
  ...
  async getDemoProducts(req, res) {
    try {
      const { count = 3 } = req.query;
      const products = await productService.getDemoProducts(parseInt(count));
      ...
      console.error('Get demo products error:', error);
      res.status(500).json({
        success: false,
        error: 'Failed to get demo products'
      });
    }
  },
